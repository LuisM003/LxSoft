...
<body>
    <div class="container">
        ...
        $encriptada3 = sha1(md5($passwordUsuarioSolicitado));
        $estado = "activo";
        ...
        if(!preg_match('/^\w+([.-_+]?\w+)*@\w+([.-]?\w+)*(\.\w{2,10})+$/',"$mailUsuarioSolicitado")){
            ...
            echo " hola: ".$encriptada3;
        }

        include_once('db.php');
        $conectar=conn();
        $sql="INSERT INTO `usuarios` VALUES ('', '$estado' ,'$nombreUsuarioSolicitado', '$mailUsuarioSolicitado', '$documentoUsuarioSolicitado', '$encriptada3',  current_timestamp(), '' , NULL , NULL)";
